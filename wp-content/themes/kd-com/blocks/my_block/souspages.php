<?php
/**
 * Template pour le bloc "Sous-pages"
 * Utilise le champ ACF 'couleur_bloc' pour appliquer les couleurs
 */

// Récupération des champs ACF
$parent_page_id        = get_field('page_parente');
...
// Préparer les variables CSS dynamiques — exposées sur le conteneur
$css_vars = array();
if (!empty($couleur_fond)) {
    $css_vars[] = '--sous-pages-bg: ' . esc_attr($couleur_fond);
}
if (!empty($couleur_texte)) {
    $css_vars[] = '--sous-pages-text: ' . esc_attr($couleur_texte);
}
if (!empty($couleur_bouton)) {
    $css_vars[] = '--sous-pages-primary: ' . esc_attr($couleur_bouton);
} else {
    // valeur par défaut si pas de couleur bouton fournie
    $css_vars[] = '--sous-pages-primary: #0089ab';
}
// Couleur de titre de bloc (H2) - toujours définir la variable CSS
$css_vars[] = '--sous-pages-block-heading: ' . esc_attr($couleur_titre_block ?: '');

// Récupérer le champ couleur_bloc (injecté automatiquement dans tous les blocs ACF)
$block_color_slug = get_field('couleur_bloc');

// Si une couleur de titre est fournie, l'exposer
if (!empty($couleur_titre)) {
    $css_vars[] = '--sous-pages-heading: ' . esc_attr($couleur_titre);
}
// Si une couleur de liens est fournie, l'exposer
if (!empty($couleur_liens)) {
    $css_vars[] = '--sous-pages-link: ' . esc_attr($couleur_liens);
}

// Note: Les variables CSS --sous-pages-heading et --sous-pages-link ont la priorité
// sur les classes has-couleur-*-color
...
if ($subpages_query->have_posts()) :
    ...
    echo '<div class="' . esc_attr($all_classes) . '"' . $style_attr . '>';

    // Titre parent
    if ($afficher_titre_parent) {
        $parent_page = get_post($parent_page_id);
        if ($parent_page) {
            echo '<h2 class="sous-pages-parent-title has-' . esc_attr($couleur_titre_block) . '-color">' . esc_html($parent_page->post_title) . '</h2>';
        }
    }
        ...
        <article class="sous-page-card has-<?php echo esc_attr($couleur_fond); ?>-background-color" <?php echo $animation_type !== 'none' ? 'data-aos="'.esc_attr($animation_type).'" data-aos-delay="'.esc_attr($delay).'"' : ''; ?>>
            ...
            <div class="sous-page-content">
                ...
                <div class='sous-page-link-wrapper'>
                    <a class="sous-page-link btn-sous-page has-<?php echo esc_attr($couleur_liens);?>-color has-<?php echo esc_attr($couleur_fond_lien); ?>-background-color" href="<?php the_permalink(); ?>">
    ...
    echo '<p class="text-gray-600">Aucune sous-page trouvée.</p>';
endif;
